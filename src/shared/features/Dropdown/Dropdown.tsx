...
import { useDropdown, useDropdownCtx, useDropdownItem } from "./useDropdown";
    ...
    <AnimatePresence>
      {isOpen && (
        <FramerAnimatePosition
          ...
          exit="hidden"
          variants={{ hidden: { y: 5 } }}
          className="absolute drop-shadow z-50 w-full top-[calc(100%+0.25rem)] right-0 max-h-80 min-w-50"
        >
          <ul
            className={cn(
